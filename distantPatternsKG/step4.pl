...
# required script location: the directory containing filtered 'vcf.gz' files from experiments
# run: ./step4.pl max_distance_to_dinucleotide[e.g. 500] OUTDISTANCES.real

@dinuc=("CC","TT","AA","GG","CT","TC","GA","AG");

open (O, ">$ARGV[1].$ARGV[0].FCON");
print O "SampleID\tDinucleotide\tChange";
...
{
@F=();
if ($file=~/!SYMB!/) {@F=split(/!SYMB!/,$file)}		#set the combination of symbols (!SYMB!) to parse gzipped VCF file name into the array, the first element of array must be tumor ID
$smp=$F[0];
