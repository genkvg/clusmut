...
# required script location: the directory containing table.* [see step3.pl], 'patterns' and step4.patts files
# run example: ./step4.pl table.differentReadMut.summ step4.patts table.differentReadMut 
...
foreach $dir (@dirs)
{
open (L, "find $dir -type f -name '$ARGV[2]' |");
while (<L>)
{
chomp;
$file=$_;
